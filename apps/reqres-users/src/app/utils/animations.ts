...
export const slideInAnimation = trigger('routeAnimations', [
  transition('LeftPage => RightPage', [
    ...
    group([
      query(':leave', [animate('300ms ease-out', style({ transform: distances.left }))]),
      query(':enter', [animate('300ms ease-out', style({ transform: distances.normalX }))]),
    ]),
  ]),
  ...
  transition('RightPage => LeftPage', [
    ...
    group([
      query(':leave', [animate('300ms ease-out', style({ transform: distances.right }))]),
      query(':enter', [animate('300ms ease-out', style({ transform: distances.normalX }))]),
    ]),
  ]),
]);
